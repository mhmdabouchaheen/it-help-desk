...
import {AdminOnlyRoute,ProtectedRoute,PublicOnlyRoute,ReportsRoute,SupportOnlyRoute} from './routes/RouteGuards'
import './App.css'
import './auth.css'
import './audit.css'
...
const TeamManagementPage=lazy(()=>import('./pages/TeamManagementPage').then(x=>({default:x.TeamManagementPage})))
const RoleManagementPage=lazy(()=>import('./pages/RoleManagementPage').then(x=>({default:x.RoleManagementPage})))
...
export default function App(){return <Routes><Route element={<PublicOnlyRoute/>}><Route path="/login" element={<LoginPage/>}/><Route path="/register" element={<RegisterPage/>}/><Route path="/forgot-password" element={<ForgotPasswordPage/>}/><Route path="/reset-password" element={<ResetPasswordPage/>}/></Route><Route element={<ProtectedRoute/>}><Route path="/app" element={<AppLayout/>}><Route index element={<Navigate to="home" replace/>}/><Route path="home" element={route(<HomePage/>)}/><Route path="profile" element={route(<ProfilePage/>)}/><Route path="tickets" element={route(<TicketListPage/>)}/><Route path="tickets/new" element={route(<CreateTicketPage/>)}/><Route path="tickets/:id" element={route(<TicketDetailPage/>)}/><Route path="tickets/:id/edit" element={route(<EditTicketPage/>)}/><Route path="notifications" element={route(<NotificationsPage/>)}/><Route element={<ReportsRoute/>}><Route path="reports" element={route(<ReportsPage/>)}/></Route><Route element={<SupportOnlyRoute/>}><Route path="activity" element={route(<ActivityLogPage/>)}/></Route><Route element={<AdminOnlyRoute/>}><Route path="team-management" element={route(<TeamManagementPage/>)}/><Route path="role-management" element={route(<RoleManagementPage/>)}/></Route></Route></Route><Route path="/" element={<Navigate to="/app/home" replace/>}/><Route path="*" element={<NotFoundPage/>}/></Routes>}
