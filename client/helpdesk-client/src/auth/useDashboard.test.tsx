...
describe('useDashboard',()=>{
  ...
  it('refetches on focus after the initial request completes',async()=>{getDashboardAsync.mockResolvedValue(dashboard);const {result}=renderHook(()=>useDashboard());await waitFor(()=>expect(result.current.isLoading).toBe(false));act(()=>window.dispatchEvent(new Event('focus')));await waitFor(()=>expect(getDashboardAsync).toHaveBeenCalledTimes(2))})
})
