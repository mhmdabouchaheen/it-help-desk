...
public sealed class ActivityLogEndpointIntegrationTests
{
    ...
    [Fact]public async Task TicketActivity_UsesJwtVisibilityPathAndHasNoIdentityQuery(){await using var f=new AuthApiFactory();var user=Guid.NewGuid();var response=await f.CreateAuthorizedClient(user,AppRoles.Employee).GetAsync($"/api/tickets/{AuthApiFactory.TicketId}/activity");Assert.Equal(HttpStatusCode.OK,response.StatusCode);f.TicketService.Verify(x=>x.GetByIdAsync(AuthApiFactory.TicketId,It.Is<TicketAccessContext>(a=>a.UserId==user),It.IsAny<CancellationToken>()));f.ActivityLogService.Verify(x=>x.GetForTicketAsync(AuthApiFactory.TicketId,It.IsAny<CancellationToken>()));}
