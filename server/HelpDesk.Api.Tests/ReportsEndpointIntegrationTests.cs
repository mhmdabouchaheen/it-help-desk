...
public sealed class ReportsEndpointIntegrationTests
{
    ...
    [Theory][InlineData(AppRoles.Employee,"pdf")][InlineData(AppRoles.Employee,"excel")][InlineData(AppRoles.Manager,"pdf")][InlineData(AppRoles.Manager,"excel")]
    ...
    public async Task SupportStaff_CanReadReportsAndFiltersBind(string role)
    {
        ...
        factory.ReportService.Verify(x=>x.GetTicketReportAsync(It.Is<TicketReportRequest>(r=>r.CategoryId==1&&r.PriorityId==2&&r.StatusId==3&&r.AssignedToUserId==agent&&r.FromUtc.HasValue&&r.ToUtc.HasValue),It.IsAny<CancellationToken>()));
    }
