...
public sealed class ReportExportServiceTests
{
    ...
    public async Task Pdf_UsesEmbeddedRegularAndBoldFonts_AndProducesSafeNonEmptyFile()
    {
        ...
        var result=await context.Service.ExportTicketReportPdfAsync(context.Request,cts.Token);
        ...
        context.Reports.Verify(x=>x.GetTicketReportAsync(context.Request,cts.Token),Times.Once);
    ...
    public async Task Pdf_AcceptsRepresentativeArabicAndLatinDisplayNames()
    {
        ...
        var result=await context.Service.ExportTicketReportPdfAsync(context.Request);
        Assert.StartsWith("%PDF",Encoding.ASCII.GetString(result.Content,0,4));
    ...
    public async Task Pdf_LargeReport_ProducesMultiplePages()
    {
        ...
        var context=Create(trend:trend);var result=await context.Service.ExportTicketReportPdfAsync(context.Request);
        using var document=PdfReader.Open(new MemoryStream(result.Content));Assert.True(document.PageCount>1);
    }
    ...
    {
        var context=Create();var result=await context.Service.ExportTicketReportExcelAsync(context.Request);
        ...
        context.Reports.Verify(x=>x.GetTicketReportAsync(context.Request,It.IsAny<CancellationToken>()),Times.Once);
    }

    [Fact]
    public async Task EmptyReport_ProducesValidPdfAndWorkbook()
    {
        var reports=new Mock<IReportService>();reports.Setup(x=>x.GetTicketReportAsync(It.IsAny<TicketReportRequest>(),It.IsAny<CancellationToken>())).ReturnsAsync(new TicketReportResponse());
        var service=new ReportExportService(reports.Object,new FixedTime());var pdf=await service.ExportTicketReportPdfAsync(new());
        ...
        var excel=await service.ExportTicketReportExcelAsync(new());using var workbook=new XLWorkbook(new MemoryStream(excel.Content));Assert.Equal(6,workbook.Worksheets.Count);
    }
    ...
    private static Context Create(string agentDisplayName="Safe Agent",IReadOnlyList<TicketReportTrendResponse>? trend=null)
    {
        ...
        var reports=new Mock<IReportService>();reports.Setup(x=>x.GetTicketReportAsync(request,It.IsAny<CancellationToken>())).ReturnsAsync(data);
        return new(request,reports,new ReportExportService(reports.Object,new FixedTime()));
    }
    private sealed class FixedTime:TimeProvider{public override DateTimeOffset GetUtcNow()=>new(2026,8,17,14,30,0,TimeSpan.Zero);}
    private sealed record Context(TicketReportRequest Request,Mock<IReportService> Reports,ReportExportService Service);
}
