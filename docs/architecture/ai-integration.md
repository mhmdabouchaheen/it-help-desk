...
Provider output is parsed into a private structured contract. Category and priority names are mapped only against active backend lookups; unknown values become null. Summary text is trimmed to 1,000 characters. At most five troubleshooting suggestions of 500 characters each are returned. React renders all output as ordinary text.
